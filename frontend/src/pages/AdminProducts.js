...
export default function AdminProducts() {
  ...
  const load = () => axios.get(`${API}/api/products`, { withCredentials: true }).then(r => setProducts(r.data));
  useEffect(() => { load(); }, []);
  ...
  const handleSave = async () => {
    try {
      if (editing) {
        await axios.put(`${API}/api/products/${editing}`, form, { withCredentials: true });
        toast.success("Product updated");
      } else {
        await axios.post(`${API}/api/products`, form, { withCredentials: true });
