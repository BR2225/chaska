...
export default function TrackOrder() {
  ...
  const handleSearch = async (e) => {
    ...
    setOrder(null);
    try {
      const { data } = await axios.get(`${API}/api/orders/${orderId.trim()}`);
      setOrder(data);
    } catch {
          ...
          <div className="space-y-6 animate-fade-in-up" data-testid="track-order-result">
            ...
            <div className="bg-white rounded-2xl p-6 sm:p-8 border border-[#E3DCD2]/50 shadow-sm">
              ...
              <div className="mt-2 flex justify-between">
                <span className="text-sm text-[#5C5042]">Delivery</span>
                <span className="text-sm text-[#2C241B]">{order.delivery_address}</span>
              </div>
            </div>
          </div>
        )}
