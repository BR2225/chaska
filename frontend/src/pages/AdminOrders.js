...
export default function AdminOrders() {
  ...
  const load = () => {
    ...
    axios.get(`${API}/api/orders${params}`, { withCredentials: true }).then(r => setOrders(r.data)).catch(() => {});
  };
  useEffect(() => { load(); }, [filterStatus]);

  const updateStatus = async (orderId, status) => {
    try {
      await axios.put(`${API}/api/orders/${orderId}/status`, { status }, { withCredentials: true });
