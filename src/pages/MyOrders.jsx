...
const statusConfig = {
  ...
  payment_pending: { icon: Clock, color: "#E8A317", label: "Payment Pending" },
        ...
        {loading ? (
          ...
          <div className="space-y-4" data-testid="orders-list">
            {orders.map(o => {
              ...
              const canRetryPayment = o.payment_method === "razorpay"
                ...
                <div key={o.id} className="bg-white rounded-2xl border border-[#E3DCD2]/50 shadow-[0_4px_20px_rgb(44,36,27,0.04)] p-5 sm:p-6 transition-all hover:shadow-[0_8px_30px_rgb(44,36,27,0.08)]" data-testid={`my-order-${o.id}`}>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-[#E3DCD2]/50">
                    ...
                    <div className="flex items-center gap-3">
                      {canRetryPayment && (
                        <Link to={`/checkout?retry=${encodeURIComponent(o.id)}`} className="text-xs text-white bg-[#D96C4A] hover:bg-[#C25D3E] font-medium rounded-full px-3 py-1.5 transition-colors">
                          Retry Payment
                        </Link>
                      )}
                      <Link to={`/track-order?id=${encodeURIComponent(o.id)}`} className="text-xs text-[#D96C4A] hover:text-[#C25D3E] font-medium flex items-center gap-1 transition-colors" data-testid={`track-${o.id}`}>
